...
CreateThread(function()
    ...
    if Config.Economics.marketEvents then
    ...
    MySQL.Async.fetchAll('SELECT ingredient, quantity FROM supply_warehouse_stock', {}, function(stockData)
        ...
        MySQL.Async.fetchAll([[
            ...
            for itemName, priceData in pairs(marketData.prices) do
                ...
                -- Apply price limits
                newPrice = math.max(priceData.base * Config.Economics.dynamicPricing.minMultiplier, newPrice)
                newPrice = math.min(priceData.base * Config.Economics.dynamicPricing.maxMultiplier, newPrice)
...
function CalculatePriceFactors(itemName, data)
    ...
    -- Time-based factors
    if Config.Economics.dynamicPricing.timeMultipliers then
    ...
    -- Player count factor
    if data.playerCount >= Config.Economics.dynamicPricing.peakThreshold then
...
function StartMarketEventSystem()
    CreateThread(function()
        while true do
            ...
            if math.random() < Config.Economics.marketEventChance then
                TriggerMarketEvent()
            end
        end
    end)
