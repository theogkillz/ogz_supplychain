...
AddEventHandler('farming:sellFruit', function(fruit, amount)
    ...
    if itemCount >= amount then
        ...
        MySQL.Async.fetchAll('SELECT * FROM supply_warehouse_stock WHERE ingredient = @ingredient', {
            ['@ingredient'] = fruit:lower()
        }, function(stockResults)
            if #stockResults > 0 then
                MySQL.Async.execute('UPDATE supply_warehouse_stock SET quantity = quantity + @quantity WHERE ingredient = @ingredient', {
                    ['@quantity'] = amount,
                    ['@ingredient'] = fruit:lower()
                })
            else
                MySQL.Async.execute('INSERT INTO supply_warehouse_stock (ingredient, quantity) VALUES (@ingredient, @quantity)', {
                    ['@ingredient'] = fruit:lower(),
                    ['@quantity'] = amount
                })
            end
